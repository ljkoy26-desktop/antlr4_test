...
#### Case A: 프로젝트가 /MD (DLL) 설정인 경우 (권장) ( 노트북에서 실행 시, 시간 매우 오래걸렸음 - 현재 사용중 )
...
# 소장님이 작업해주신 db2 grammer 파일 
java -jar antlr-4.13.2-complete.jar -Dlanguage=Cpp -package antlrcpp_wv_db2 -no-listener -visitor -o wv_db2_gen wvDb2Lexer.g4
java -jar antlr-4.13.2-complete.jar -Dlanguage=Cpp -package antlrcpp_wv_db2 -no-listener -visitor -o wv_db2_gen wvDb2Parser.g4
		  


# grammer 에서 제공하는 파일 
java -jar antlr-4.13.2-complete.jar -Dlanguage=Cpp -package antlrcpp_db2 -no-listener -visitor -o db2_gen Db2Lexer.g4
java -jar antlr-4.13.2-complete.jar -Dlanguage=Cpp -package antlrcpp_db2 -no-listener -visitor -o db2_gen Db2Parser.g4
...
* 공통
- 구성속성 -> 일반 -> 출력 디렉토리  ../bin/$(Configuration) $(Platform)\
- 구성속성 -> 일반 -> 중간 디렉토리  ../obj/$(ProjectName)/$(Configuration) $(Platform)\
