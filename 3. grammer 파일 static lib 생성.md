...
DLL(동적 라이브러리)이 더 멋져 보일 수 있지만, 
ANTLR와 C++ 환경에서는 치명적인 단점이 있습니다.
...
복잡도: ANTLR 생성 클래스들에 __declspec(dllexport) 키워드를 붙이거나, 
래퍼(Wrapper) 클래스를 따로 만들어야 하는데 작업량이 엄청납니다.
...
# 3단계: MFC 프로젝트(ANTLR4TEST)와 연결하기

**MFC 프로젝트(ANTLR4TEST)**의 속성을 엽니다.

# C/C++ -> 일반 -> 추가 포함 디렉터리에 SqlParserLib 추가 

링커(Linker) -> 일반 -> 추가 라이브러리 디렉터리에 SqlParserLib의 빌드 출력 경로
$(SolutionDir)$(Platform)\$(Configuration)\ 추가.
...
# 각 db별 산출물 경로 연결 (  C/C++ -> 일반 -> 추가 포함 디렉터리  )
